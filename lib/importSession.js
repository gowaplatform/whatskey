...
async function importEvolutionSession(options = {}) {
    ...
    if (!evolutionApiUrl) throw new Error('EVOLUTION_API_URL é obrigatório.');
    ...
    logger.info('Reiniciando a conexão no Evolution API...');
    try {
        ...
        logger.error(`Erro ao chamar a API do Evolution para conectar: ${message}`);
        logger.warn('Você pode tentar conectar manualmente no Evolution Manager.');
    }
